...
All configuration is done in the <tt>conf</tt> directory via environment yaml files. Copy <tt>development.yml.example</tt> to <tt>deelopment.yml</tt> within that directory and alter the settings to match your local environment.

There are a couple additional steps you'll need to take to get rdoc.info working:

* Install Jeff Rafter's fork of YARD -- see http://github.com/jeffrafter/yard
